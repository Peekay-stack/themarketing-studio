...
3. ~~`POST /idea-platform`~~ — **my error: it has existed since handover 1.** Now wired properly: adopt
   posts to it, `GET /idea-platform` hydrates the tab on entry, and the payload carries `source`
   (`"model"` until the drafted line is edited), `skipped`, and the id of the brief it answers. The five
   tests are no longer held in client state.
