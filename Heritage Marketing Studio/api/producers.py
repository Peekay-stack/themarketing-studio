...
def carousel_concept(objective: str, house: dict | None = None, platform: dict | None = None,
    ...
    prompt = (
        ...
        "slide. The pack itself is a real, fixed product photograph — never describe its printed "
        ...
        "the SCENE (their own face, a caption, a sign) — never printed onto the product. Whenever a "
        ...
        "slides; that is the one place it has to hold. Never describe a sign, chalkboard, caption, "
        "nameplate, label or any other on-image text or lettering appearing anywhere in a slide — the "
        "ON-SLIDE HEADLINE ALREADY CARRIES that words-for-years, name or fact as real, correctly-"
        "spelled overlay text added afterward; asking the image itself to also show writing produces "
        "garbled, misspelled nonsense, because an image model cannot render legible text. If a "
        "recurring person's role in the story depends on them being an outsider or visitor — a "
        "delivery person, a vendor, anyone who does not live in the home — keep that role visibly "
        "legible in EVERY present-day appearance, not only their locked face: a doorway or threshold, "
        "their crate or work tool, a distinct uniform-like detail. A person shown fully inside a "
        "home's interior with no trace of their actual role reads as the resident, not the visitor — "
        "don't let a slide accidentally imply they live there when the whole story depends on them "
        "not. When a person's locked description includes sleepwear, a robe, a nightshirt or other "
        "loose/unstructured clothing, never pair it with a pose that bends, crouches or reaches down "
        "low — have them stand, kneel with a straight back, or reach at chest height instead; a deep "
        "bend in loose clothing is a common image-model failure where the hem rides up.\n"
