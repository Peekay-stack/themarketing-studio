...
def _add_footer(doc):
    """The Marketing Studio's own mark + URL on every page of every document this module builds
    (guided briefs, the production bible, messaging house, idea platform, comm plan, call sheets,
    scripts) — every one of them leaves the portal and gets circulated on its own. Small and quiet: a
    footer competing with the document's own content would be the wrong call for a white-label
    deliverable. Shared with brief_render.py's own copy of this same helper — same asset, same look."""
