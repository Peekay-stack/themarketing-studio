...
4. NeedScope brand positioning section with an embedded wheel as `Figure 1` (built either from the user's uploaded NeedScope chart or constructed from communications cues).
5. CB/CA → DB/DA section with the embedded shift diagram as `Figure 2`.
6. Single Minded Proposition section with a green-bordered callout and a "why only this brand can say it" defence.
7. Pricing, distribution, and content snapshot.
8. Opportunities and threats table.
9. Recommended actions, every one of which ladders to the SMP and respects the NeedScope discipline.
10. Caveats that name exactly what to validate before externalising.
...
3. **Build the analytic core.** In this order: (a) competitor snapshot, (b) messaging matrix, (c) NeedScope positions for every relevant brand, (d) CB/CA → DB/DA for the focal brand, (e) SMP triangulation. The SMP is the last analytic step because it must reconcile NeedScope + CB/CA → DB/DA + competitive whitespace.
...
- Be 5 to 7 pages on US Letter at default body 11pt.
