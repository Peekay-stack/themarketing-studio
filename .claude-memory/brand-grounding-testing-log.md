...
tab, to the user's own specified "works from pre-trained knowledge alone, not the plan or platform";
re-verified Round 12's pack fix fresh with a 3/3 clean batch; explained the literal prompt reaching the
image generator at the user's request); Round 14 ("Include a recurring model/cast" did nothing when
